...
struct SecureStorage {
    ...
    selected_talkecho_model: Option<String>,
}
...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "talkecho_license_key" => storage.license_key = Some(item.value),
            "talkecho_instance_id" => storage.instance_id = Some(item.value),
            "selected_talkecho_model" => storage.selected_talkecho_model = Some(item.value),
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    Ok(StorageResult {
        ...
        selected_talkecho_model: storage.selected_talkecho_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "talkecho_license_key" => storage.license_key = None,
            "talkecho_instance_id" => storage.instance_id = None,
            "selected_talkecho_model" => storage.selected_talkecho_model = None,
...
pub async fn validate_license_api(_app: AppHandle) -> Result<ValidateResponse, String> {
    // Modified: Always return active license to unlock all features
    // No need to call remote API or check credentials
    Ok(ValidateResponse {
        is_active: true,
