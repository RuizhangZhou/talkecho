// TalkEcho Linux speaker input and stream
...
pub fn get_input_devices() -> Result<Vec<AudioDevice>> {
    ...
    let mut context = Context::new(&mainloop, "talkecho-device-enum")
    ...
    let op = introspector.get_source_info_list(move |result| match result {
        pulse::callbacks::ListResult::Item(info) => {
            if let Some(name) = &info.name {
                // Exclude monitor sources from input list.
...
fn sink_to_monitor_source(sink_name: &str) -> String {
...
impl SpeakerInput {
    pub fn new(device_id: Option<String>) -> Result<Self> {
        // For Linux, device_id is the PulseAudio sink name.
        let sink_name = device_id.filter(|id| !id.is_empty() && id != "default");
        Ok(Self { sink_name })
    }
    ...
    pub fn stream(self) -> SpeakerStream {
        ...
        let capture_thread = thread::spawn(move || {
            if let Err(e) =
                SpeakerStream::capture_audio_loop(queue_clone, waker_clone, sink_name.as_deref(), init_tx)
            {
                error!("[capture_thread] audio capture loop failed: {}", e);
            }
        });

        let actual_sample_rate = match init_rx.recv() {
            Ok(Ok(sr)) => sr,
            Ok(Err(e)) => {
                error!("[stream] audio initialization failed: {}", e);
                DEFAULT_SAMPLE_RATE
            }
            Err(e) => {
                error!("[stream] failed to receive audio init signal: {}", e);
                DEFAULT_SAMPLE_RATE
            }
        };
...
impl SpeakerStream {
    ...
    fn capture_audio_loop(
        ...
        let monitor_source = sink_name.map(sink_to_monitor_source);
        let final_source = monitor_source.or_else(get_default_monitor_source);
                        ...
                        Ok(_) => {
                            ...
                            if !samples.is_empty() {
                                let dropped = {
                                    ...
                                    let max_buffer_size = 131_072; // 128KB buffer (matching macOS/Windows)
                                ...
                                if dropped > 0 {
                                    warn!(
                                        "[capture_audio_loop] Linux buffer overflow - dropped {} samples",
                                        dropped
                                    );
                                }
